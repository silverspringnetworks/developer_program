...
{
    /// <summary>
    /// Representation of a CoAP resource related to a device.
    /// </summary>
    public class CoApResource
    {
        private string __Uri = "unknown";
        private CoApLinkAttributes __LinkAttributes;
        private bool __IsObservable = false;
        ...
        public CoApResource(string resource)
        {
            string[] linkAttributes = resource.Split(';');
            __LinkAttributes = new CoApLinkAttributes(resource);
            __Uri = linkAttributes[0];
            __Uri = __Uri.Substring(1, __Uri.Length - 2);    // strip off the <> container
            foreach(CoApLinkAttribute a in __LinkAttributes)
            {
                if (a.Name == LinkAttribute.Observable)
                {
                    __IsObservable = (bool)a.Value;
                }
            }
        }
        /// <summary>
        /// The URI for the attribute (e.g. /time).
        /// </summary>
        ...
        /// <summary>
        /// Whether or not the observerable attribute is owned for the resource.
        /// </summary>
        public bool IsObservable
        {
            get { return __IsObservable; }
            set { }
        }
        /// <summary>
        /// A list of all attributes related to the resource.
        /// </summary>
        public CoApLinkAttributes LinkAttributes
        {
            get { return __LinkAttributes; }
            set { }
        }
    }
